...
<div  >
	<h2>Productos disponibles</h2>
		...
		for($a = 0; $a < sizeof($productos); $a++){
				...
				echo " onClick='agregarAVenta( " .  $productWithPrice["id_producto"] . " )' ";
				...
				echo " " . moneyFormat( $productWithPrice["precio_venta"] ) .  "<br>";
				echo "</td>";
			}
...
<script>
	...
	function doMath(  ){
		...
		for (var i=0; i < carritoDeCompra.length; i++) {
			...
			var sub_total_a_compra = (jQuery("#item-cantidad-"+id).val()) * parseFloat(carritoDeCompra[i].precio_compra);

			final_weight += parseFloat( jQuery("#item-cantidad-"+id).val() - jQuery("#item-descuento-"+id).val());
			estimated_earnings_at_sell_time += sub_total_a_compra;
			...
			jQuery("#rendimeinto-alaventa-"+ id).html(cf(carritoDeCompra[i].precio_compra)) ; 			//a la venta
			
			var rendimiento = sub_total - sub_total_a_compra;
	...
	function agregarAVenta( id_producto ){
		...
		html += td( "<input id='item-precio-"+id_producto+"' onKeyUp='doMath(  )' value='"+ prod.precio_venta +"' type=text>" );
...
<div id="rendimientos" style="display:none" >
	<h2>Rendimiento para esta venta</h4>
		<table style="width:100%">
	    <tr id="TablaDeRendimientos">
	        <td>Producto</td>
	        <td>Importe de producto</td>
	        <td>Precio a la compra actual</td>  
	        <td>Rendimiento</td>	
	    </tr>
	    <tr >
	        <td></td>
...
<div id="readyToBuy" style="display:none" align=center>
	<h4><input type=button value="Realizar la venta" onClick="doSell()"></h4>
</div>
