...
abstract class GastosDAOBase extends TablaDAO
{
	...
	/**
	  ...
	  **/
	public static final function search( $gastos , $json = false)
	{
	...
	{
		$sql = "UPDATE gastos SET  folio = ?, concepto = ?, monto = ?, fecha = ?, fecha_ingreso = ?, id_sucursal = ?, id_usuario = ? WHERE  id_gasto = ?;";
		$params = array( 
			$gastos->getFolio(), 
			$gastos->getConcepto(), 
			$gastos->getMonto(), 
	...
	{
		$sql = "INSERT INTO gastos ( folio, concepto, monto, fecha, fecha_ingreso, id_sucursal, id_usuario ) VALUES ( ?, ?, ?, ?, ?, ?, ?);";
		$params = array( 
			$gastos->getFolio(), 
			$gastos->getConcepto(), 
			$gastos->getMonto(), 
	...
	/**
	  ...
	  * @param bool [$json] Verdadero para obtener los resultados en forma JSON y no objetos. En caso de no presentare este parametro se tomara el valor default de false.
	  **/
	public static final function byRange( $gastosA , $gastosB , $json = false)
	{
		$sql = "SELECT * from gastos WHERE ("; 
		$val = array();
