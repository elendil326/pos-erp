...
abstract class PrecioProductoRolDAOBase extends DAO
{
	...
	{
		if( ! is_null ( self::getByPK(  $precio_producto_rol->getIdProducto() , $precio_producto_rol->getIdRol() ) ) )
		{
	...
	public static final function search( $precio_producto_rol , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( ! is_null( $precio_producto_rol->getIdProducto() ) ){
			$sql .= " id_producto = ? AND";
			array_push( $val, $precio_producto_rol->getIdProducto() );
		}

		if( ! is_null( $precio_producto_rol->getIdRol() ) ){
			$sql .= " id_rol = ? AND";
			array_push( $val, $precio_producto_rol->getIdRol() );
		}

		if( ! is_null( $precio_producto_rol->getPrecioUtilidad() ) ){
			$sql .= " precio_utilidad = ? AND";
			array_push( $val, $precio_producto_rol->getPrecioUtilidad() );
		}

		if( ! is_null( $precio_producto_rol->getEsMargenUtilidad() ) ){
	...
	public static final function byRange( $precio_producto_rolA , $precio_producto_rolB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from precio_producto_rol WHERE ("; 
		$val = array();
		if( ( !is_null (($a = $precio_producto_rolA->getIdProducto()) ) ) & ( ! is_null ( ($b = $precio_producto_rolB->getIdProducto()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_rolA->getIdRol()) ) ) & ( ! is_null ( ($b = $precio_producto_rolB->getIdRol()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_rolA->getPrecioUtilidad()) ) ) & ( ! is_null ( ($b = $precio_producto_rolB->getPrecioUtilidad()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_rolA->getEsMargenUtilidad()) ) ) & ( ! is_null ( ($b = $precio_producto_rolB->getEsMargenUtilidad()) ) ) ){
	...
	{
		if( is_null( self::getByPK($precio_producto_rol->getIdProducto(), $precio_producto_rol->getIdRol()) ) ) throw new Exception('Campo no encontrado.');
