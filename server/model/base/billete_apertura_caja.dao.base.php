...
abstract class BilleteAperturaCajaDAOBase extends DAO
{
		...
		private static function recordExists(  $id_billete, $id_apertura_caja ){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_apertura_caja . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_billete, $id_apertura_caja){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_apertura_caja . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_billete, $id_apertura_caja ){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_apertura_caja . "-";
	...
	{
		if(  self::getByPK(  $billete_apertura_caja->getIdBillete() , $billete_apertura_caja->getIdAperturaCaja() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_billete, $id_apertura_caja)){
			return self::getRecord( $id_billete, $id_apertura_caja );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_billete, $id_apertura_caja );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_billete
			//id_apertura_caja
    		self::pushRecord( $bar, $foo["id_billete"],$foo["id_apertura_caja"] );
		}
		return $allData;
	}
	...
	public static final function search( $billete_apertura_caja , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $billete_apertura_caja->getIdBillete() != NULL){
			$sql .= " id_billete = ? AND";
			array_push( $val, $billete_apertura_caja->getIdBillete() );
		}

		if( $billete_apertura_caja->getIdAperturaCaja() != NULL){
			$sql .= " id_apertura_caja = ? AND";
			array_push( $val, $billete_apertura_caja->getIdAperturaCaja() );
		}

		if( $billete_apertura_caja->getCantidad() != NULL){
			$sql .= " cantidad = ? AND";
	...
	public static final function byRange( $billete_apertura_cajaA , $billete_apertura_cajaB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from billete_apertura_caja WHERE ("; 
		$val = array();
		if( (($a = $billete_apertura_cajaA->getIdBillete()) !== NULL) & ( ($b = $billete_apertura_cajaB->getIdBillete()) !== NULL) ){
		...
		if( (($a = $billete_apertura_cajaA->getIdAperturaCaja()) !== NULL) & ( ($b = $billete_apertura_cajaB->getIdAperturaCaja()) !== NULL) ){
		...
		if( (($a = $billete_apertura_cajaA->getCantidad()) !== NULL) & ( ($b = $billete_apertura_cajaB->getCantidad()) !== NULL) ){
	...
	{
		if(self::getByPK($billete_apertura_caja->getIdBillete(), $billete_apertura_caja->getIdAperturaCaja()) === NULL) throw new Exception('Campo no encontrado.');
