...
abstract class ImpuestoClasificacionClienteDAOBase extends DAO
{
	...
	{
		if( ! is_null ( self::getByPK(  $impuesto_clasificacion_cliente->getIdImpuesto() , $impuesto_clasificacion_cliente->getIdClasificacionCliente() ) ) )
		{
	...
	{
		if(  is_null( $id_impuesto ) || is_null( $id_clasificacion_cliente )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "ImpuestoClasificacionCliente-" . $id_impuesto."-" . $id_clasificacion_cliente ))){
                Logger::log("REDIS !");
                return new ImpuestoClasificacionCliente($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoClasificacionCliente-" . $id_impuesto."-" . $id_clasificacion_cliente, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoClasificacionCliente-" . $bar->getIdImpuesto()."-" . $bar->getIdClasificacionCliente(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $impuesto_clasificacion_clienteA , $impuesto_clasificacion_clienteB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new ImpuestoClasificacionCliente($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoClasificacionCliente-" . $bar->getIdImpuesto()."-" . $bar->getIdClasificacionCliente(), $bar );
		}
		return $ar;
	}
	...
	{
		if( is_null( self::getByPK($impuesto_clasificacion_cliente->getIdImpuesto(), $impuesto_clasificacion_cliente->getIdClasificacionCliente()) ) ) throw new Exception('Campo no encontrado.');
