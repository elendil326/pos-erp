...
abstract class ImpuestoSucursalDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_impuesto ) || is_null( $id_sucursal )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "ImpuestoSucursal-" . $id_impuesto."-" . $id_sucursal ))){
                Logger::log("REDIS !");
                return new ImpuestoSucursal($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoSucursal-" . $id_impuesto."-" . $id_sucursal, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoSucursal-" . $bar->getIdImpuesto()."-" . $bar->getIdSucursal(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $impuesto_sucursalA , $impuesto_sucursalB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new ImpuestoSucursal($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "ImpuestoSucursal-" . $bar->getIdImpuesto()."-" . $bar->getIdSucursal(), $bar );
		}
		return $ar;
	}
