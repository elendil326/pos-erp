...
abstract class ReglaDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_regla )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "Regla-" . $id_regla ))){
                Logger::log("REDIS !");
                return new Regla($obj);
            }
		...
		$foo = new Regla( $rs );
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "Regla-" . $id_regla, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "Regla-" . $bar->getIdRegla(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $reglaA , $reglaB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new Regla($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "Regla-" . $bar->getIdRegla(), $bar );
		}
		return $ar;
	}
