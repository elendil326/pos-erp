...
abstract class AperturaCajaDAOBase extends DAO
{
		...
		private static function recordExists(  $id_apertura_caja ){
			$pk = "";
			$pk .= $id_apertura_caja . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_apertura_caja){
			$pk = "";
			$pk .= $id_apertura_caja . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_apertura_caja ){
			$pk = "";
			$pk .= $id_apertura_caja . "-";
	...
	{
		if(  self::getByPK(  $apertura_caja->getIdAperturaCaja() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_apertura_caja)){
			return self::getRecord( $id_apertura_caja );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_apertura_caja );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_apertura_caja
    		self::pushRecord( $bar, $foo["id_apertura_caja"] );
		}
		return $allData;
	}
	...
	public static final function byRange( $apertura_cajaA , $apertura_cajaB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from apertura_caja WHERE ("; 
		$val = array();
		if( (($a = $apertura_cajaA->getIdAperturaCaja()) !== NULL) & ( ($b = $apertura_cajaB->getIdAperturaCaja()) !== NULL) ){
		...
		if( (($a = $apertura_cajaA->getIdCaja()) !== NULL) & ( ($b = $apertura_cajaB->getIdCaja()) !== NULL) ){
		...
		if( (($a = $apertura_cajaA->getFecha()) !== NULL) & ( ($b = $apertura_cajaB->getFecha()) !== NULL) ){
		...
		if( (($a = $apertura_cajaA->getSaldo()) !== NULL) & ( ($b = $apertura_cajaB->getSaldo()) !== NULL) ){
		...
		if( (($a = $apertura_cajaA->getIdCajero()) !== NULL) & ( ($b = $apertura_cajaB->getIdCajero()) !== NULL) ){
	...
	{
		if(self::getByPK($apertura_caja->getIdAperturaCaja()) === NULL) throw new Exception('Campo no encontrado.');
