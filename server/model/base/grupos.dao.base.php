...
abstract class GruposDAOBase extends DAO
{
		...
		private static function recordExists(  $id_grupo ){
			$pk = "";
			$pk .= $id_grupo . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_grupo){
			$pk = "";
			$pk .= $id_grupo . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_grupo ){
			$pk = "";
			$pk .= $id_grupo . "-";
	...
	public static final function save( &$grupos )
	{
		...
		{
			try{ return GruposDAOBase::update( $grupos) ; } catch(Exception $e){ throw $e; }
		}else{
			try{ return GruposDAOBase::create( $grupos) ; } catch(Exception $e){ throw $e; }
		}
	}
	...
	{
		if(self::recordExists(  $id_grupo)){
			return self::getRecord( $id_grupo );
		}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_grupo
    		self::pushRecord( $bar, $foo["id_grupo"] );
		}
		return $allData;
	}
	...
	public static final function byRange( $gruposA , $gruposB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from grupos WHERE ("; 
		$val = array();
		if( (($a = $gruposA->getIdGrupo()) != NULL) & ( ($b = $gruposB->getIdGrupo()) != NULL) ){
		...
		if( (($a = $gruposA->getNombre()) != NULL) & ( ($b = $gruposB->getNombre()) != NULL) ){
		...
		if( (($a = $gruposA->getDescripcion()) != NULL) & ( ($b = $gruposB->getDescripcion()) != NULL) ){
