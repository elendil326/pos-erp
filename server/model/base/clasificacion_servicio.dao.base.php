...
abstract class ClasificacionServicioDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_clasificacion_servicio )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "ClasificacionServicio-" . $id_clasificacion_servicio ))){
                Logger::log("REDIS !");
                return new ClasificacionServicio($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "ClasificacionServicio-" . $id_clasificacion_servicio, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "ClasificacionServicio-" . $bar->getIdClasificacionServicio(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $clasificacion_servicioA , $clasificacion_servicioB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new ClasificacionServicio($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "ClasificacionServicio-" . $bar->getIdClasificacionServicio(), $bar );
		}
		return $ar;
	}
