...
abstract class ChequeAbonoCompraDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_cheque ) || is_null( $id_abono_compra )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "ChequeAbonoCompra-" . $id_cheque."-" . $id_abono_compra ))){
                Logger::log("REDIS !");
                return new ChequeAbonoCompra($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoCompra-" . $id_cheque."-" . $id_abono_compra, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoCompra-" . $bar->getIdCheque()."-" . $bar->getIdAbonoCompra(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $cheque_abono_compraA , $cheque_abono_compraB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new ChequeAbonoCompra($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoCompra-" . $bar->getIdCheque()."-" . $bar->getIdAbonoCompra(), $bar );
		}
		return $ar;
	}
