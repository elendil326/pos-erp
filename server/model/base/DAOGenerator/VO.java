...
class VO{
	...
	*/
	...
	{
		System.out.println( "table: " + t_name );
	...
	*/
	...
	{
		System.out.println( "view: " + vname  );

		String asSelect = br.readLine();

		asSelect = asSelect.substring(10);

		String [] tokens = asSelect.split(",");
		...
		for( String t : tokens )
		{
			...
			String [] foo = t.trim().split("AS");
			
			String [] bar = foo[ foo.length-1 ].trim().split("\\.");
			
			
			
			String title = bar[ bar.length-1 ].trim().substring( 1 , bar[ bar.length-1 ].trim().length()-1);
		...
		parseViewTableVO( vname, fields );
		parseViewTableADO( vname, fields );
	}
	...
	static void parseViewTableVO( String tabla, ArrayList<Field> fields ) throws IOException
	{
		...
		pw.println("<?php");
		pw.println("/** Value Object file for View "+ tabla +"." );
		pw.println("  * ");
		pw.println("  * VO objects for views does not have any behaviour except for retrieval of its own data (accessors).");
		pw.println("  * @author Alan Gonzalez <alan@caffeina.mx> ");
		pw.println("  * @access public");
		pw.println("  * ");
		pw.println("  */");
		...
		for( Field f : fields)
		{
			...
			pw.println( "	  * get" + camelCaseCampo );
			pw.println( "	  * " );
			pw.println( "	  * Get the <i>" + f.title + "</i> property for this "+ camelCaseTabla +" object."  );
			pw.println( "	  * @return "+ f.type );
			pw.println( "	  */");
			...
			pw.println( "	/**");
			pw.println( "	  * set" + camelCaseCampo );
			pw.println( "	  * " );
			pw.println( "	  * Set the <i>" + f.title + "</i> property for this "+ camelCaseTabla +" object."  );
			pw.println( "	  * @param "+ f.type );
			pw.println( "	  */");
	...
	static void parseViewTableADO( String tabla, ArrayList<Field> fields ) throws IOException
	{
		...
		pw.println("  * ");
		pw.println("  * Esta clase contiene toda la manipulacion de bases de datos que se necesita para ");
		pw.println("  * <b>recuperar</b> instancias de objetos {@link "+toCamelCase(tabla)+" }. ");
		pw.println("  * @author Alan Gonzalez <alan@caffeina.mx> ");
		pw.println("  * @access public");
		pw.println("  * ");
		pw.println("  */");		
		...
		//dao base
		//inicia -------
		 fileName = "base/" + tabla + ".dao.base.php";
		
		 className = toCamelCase( tabla );
		...
		pw.println("abstract class " + className+ "DAOBase extends VistaDAO" );		
		pw.println("{");
		pw.println();
	...
	*/
	...
	static void parseContent() throws IOException
	{
		...
		while( (line = br.readLine()) != null )
		{
			...
			if( line.indexOf("CREATE OR REPLACE VIEW") != -1 ) 
			{
				views_count++;

				String v_name = line.split(" ")[ line.split(" ").length - 1 ];
