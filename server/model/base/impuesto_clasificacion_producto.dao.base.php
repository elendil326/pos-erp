...
abstract class ImpuestoClasificacionProductoDAOBase extends DAO
{
		...
		private static function recordExists(  $id_impuesto, $id_clasificacion_producto ){
			$pk = "";
			$pk .= $id_impuesto . "-";
			$pk .= $id_clasificacion_producto . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_impuesto, $id_clasificacion_producto){
			$pk = "";
			$pk .= $id_impuesto . "-";
			$pk .= $id_clasificacion_producto . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_impuesto, $id_clasificacion_producto ){
			$pk = "";
			$pk .= $id_impuesto . "-";
			$pk .= $id_clasificacion_producto . "-";
	...
	{
		if(  self::getByPK(  $impuesto_clasificacion_producto->getIdImpuesto() , $impuesto_clasificacion_producto->getIdClasificacionProducto() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_impuesto, $id_clasificacion_producto)){
			return self::getRecord( $id_impuesto, $id_clasificacion_producto );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_impuesto, $id_clasificacion_producto );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		self::pushRecord( $bar, $foo["id_impuesto"],$foo["id_clasificacion_producto"] );
		}
		return $allData;
	}
	...
	public static final function search( $impuesto_clasificacion_producto , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $impuesto_clasificacion_producto->getIdImpuesto() != NULL){
			$sql .= " id_impuesto = ? AND";
			array_push( $val, $impuesto_clasificacion_producto->getIdImpuesto() );
		}

		if( $impuesto_clasificacion_producto->getIdClasificacionProducto() != NULL){
	...
	public static final function byRange( $impuesto_clasificacion_productoA , $impuesto_clasificacion_productoB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from impuesto_clasificacion_producto WHERE ("; 
		$val = array();
		if( (($a = $impuesto_clasificacion_productoA->getIdImpuesto()) !== NULL) & ( ($b = $impuesto_clasificacion_productoB->getIdImpuesto()) !== NULL) ){
		...
		if( (($a = $impuesto_clasificacion_productoA->getIdClasificacionProducto()) !== NULL) & ( ($b = $impuesto_clasificacion_productoB->getIdClasificacionProducto()) !== NULL) ){
	...
	{
		if(self::getByPK($impuesto_clasificacion_producto->getIdImpuesto(), $impuesto_clasificacion_producto->getIdClasificacionProducto()) === NULL) throw new Exception('Campo no encontrado.');
