...
abstract class ImpresoraDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_impresora )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "Impresora-" . $id_impresora ))){
                Logger::log("REDIS !");
                return new Impresora($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "Impresora-" . $id_impresora, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "Impresora-" . $bar->getIdImpresora(), $bar );
		}
		return $allData;
	}
	...
	private static final function create( &$impresora )
	{
		...
		if($ar == 0) return 0;
		/* save autoincremented value on obj */  $impresora->setIdImpresora( $conn->Insert_ID() ); /*  */ 
		return $ar;
	}
	...
	public static final function byRange( $impresoraA , $impresoraB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new Impresora($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "Impresora-" . $bar->getIdImpresora(), $bar );
		}
		return $ar;
	}
