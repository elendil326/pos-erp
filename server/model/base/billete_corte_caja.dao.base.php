...
abstract class BilleteCorteCajaDAOBase extends DAO
{
		...
		private static function recordExists(  $id_billete, $id_corte_caja ){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_corte_caja . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_billete, $id_corte_caja){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_corte_caja . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_billete, $id_corte_caja ){
			$pk = "";
			$pk .= $id_billete . "-";
			$pk .= $id_corte_caja . "-";
	...
	{
		if(  self::getByPK(  $billete_corte_caja->getIdBillete() , $billete_corte_caja->getIdCorteCaja() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_billete, $id_corte_caja)){
			return self::getRecord( $id_billete, $id_corte_caja );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_billete, $id_corte_caja );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_billete
			//id_corte_caja
    		self::pushRecord( $bar, $foo["id_billete"],$foo["id_corte_caja"] );
		}
		return $allData;
	}
	...
	public static final function byRange( $billete_corte_cajaA , $billete_corte_cajaB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from billete_corte_caja WHERE ("; 
		$val = array();
		if( (($a = $billete_corte_cajaA->getIdBillete()) !== NULL) & ( ($b = $billete_corte_cajaB->getIdBillete()) !== NULL) ){
		...
		if( (($a = $billete_corte_cajaA->getIdCorteCaja()) !== NULL) & ( ($b = $billete_corte_cajaB->getIdCorteCaja()) !== NULL) ){
		...
		if( (($a = $billete_corte_cajaA->getCantidadEncontrada()) !== NULL) & ( ($b = $billete_corte_cajaB->getCantidadEncontrada()) !== NULL) ){
		...
		if( (($a = $billete_corte_cajaA->getCantidadDejada()) !== NULL) & ( ($b = $billete_corte_cajaB->getCantidadDejada()) !== NULL) ){
	...
	{
		if(self::getByPK($billete_corte_caja->getIdBillete(), $billete_corte_caja->getIdCorteCaja()) === NULL) throw new Exception('Campo no encontrado.');
