...
abstract class LoteDAOBase extends DAO
{
		...
		private static function recordExists(  $id_lote ){
			$pk = "";
			$pk .= $id_lote . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_lote){
			$pk = "";
			$pk .= $id_lote . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_lote ){
			$pk = "";
			$pk .= $id_lote . "-";
	...
	{
		if(  self::getByPK(  $lote->getIdLote() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_lote)){
			return self::getRecord( $id_lote );
		}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_lote
    		self::pushRecord( $bar, $foo["id_lote"] );
		}
		return $allData;
	}
	...
	{
		$sql = "UPDATE lote SET  id_almacen = ? WHERE  id_lote = ?;";
		$params = array( 
			$lote->getIdAlmacen(), 
			$lote->getIdLote(), );
		global $conn;
	...
	{
		$sql = "INSERT INTO lote ( id_lote, id_almacen ) VALUES ( ?, ?);";
		$params = array( 
			$lote->getIdLote(), 
			$lote->getIdAlmacen(), 
		 );
	...
	public static final function byRange( $loteA , $loteB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from lote WHERE ("; 
		$val = array();
		if( (($a = $loteA->getIdLote()) !== NULL) & ( ($b = $loteB->getIdLote()) !== NULL) ){
		...
		if( (($a = $loteA->getIdAlmacen()) !== NULL) & ( ($b = $loteB->getIdAlmacen()) !== NULL) ){
	...
	{
		if(self::getByPK($lote->getIdLote()) === NULL) throw new Exception('Campo no encontrado.');
