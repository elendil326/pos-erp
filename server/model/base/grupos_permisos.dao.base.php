...
abstract class GruposPermisosDAOBase extends TablaDAO
{
	...
	public static final function save( &$grupos_permisos )
	{
		...
		{
			try{ return GruposPermisosDAOBase::create( $grupos_permisos) ; } catch(Exception $e){ throw $e; }
		}else{
			try{ return GruposPermisosDAOBase::update( $grupos_permisos) ; } catch(Exception $e){ throw $e; }
		}
	}
	...
	/**
	  ...
	  **/
	public static final function search( $grupos_permisos , $json = false)
	{
		...
		if($json === false){
			$ar = array();
			foreach ($rs as $foo) {
    			array_push( $ar, new GruposPermisos($foo));
			}
			return $ar;
		}else{
			$allData = '[';
			foreach ($rs as $foo) {
    			$allData .= new GruposPermisos($foo) . ',';
			}
	...
	/**
	  *	Buscar por rango.
	  *	
	  * Este metodo proporciona capacidad de busqueda para conseguir un juego de objetos {@link GruposPermisos} de la base de datos siempre y cuando 
	  * esten dentro del rango de atributos activos de dos objetos criterio de tipo {@link GruposPermisos}.
	  * 
	  ...
	  * @param bool [$json] Verdadero para obtener los resultados en forma JSON y no objetos. En caso de no presentare este parametro se tomara el valor default de false.
	  **/
	public static final function byRange( $grupos_permisosA , $grupos_permisosB , $json = false)
	{
		$sql = "SELECT * from grupos_permisos WHERE ("; 
		$val = array();
		if( (($a = $grupos_permisosA->getIdGrupo()) != NULL) & ( ($b = $grupos_permisosB->getIdGrupo()) != NULL) ){
		...
		if( (($a = $grupos_permisosA->getIdPermiso()) != NULL) & ( ($b = $grupos_permisosB->getIdPermiso()) != NULL) ){
