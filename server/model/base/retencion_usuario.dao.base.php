...
abstract class RetencionUsuarioDAOBase extends DAO
{
		...
		private static function recordExists(  $id_retencion, $id_usuario ){
			$pk = "";
			$pk .= $id_retencion . "-";
			$pk .= $id_usuario . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_retencion, $id_usuario){
			$pk = "";
			$pk .= $id_retencion . "-";
			$pk .= $id_usuario . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_retencion, $id_usuario ){
			$pk = "";
			$pk .= $id_retencion . "-";
			$pk .= $id_usuario . "-";
	...
	{
		if(  self::getByPK(  $retencion_usuario->getIdRetencion() , $retencion_usuario->getIdUsuario() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_retencion, $id_usuario)){
			return self::getRecord( $id_retencion, $id_usuario );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_retencion, $id_usuario );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_retencion
			//id_usuario
    		self::pushRecord( $bar, $foo["id_retencion"],$foo["id_usuario"] );
		}
		return $allData;
	}
	...
	public static final function search( $retencion_usuario , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $retencion_usuario->getIdRetencion() != NULL){
			$sql .= " id_retencion = ? AND";
			array_push( $val, $retencion_usuario->getIdRetencion() );
		}

		if( $retencion_usuario->getIdUsuario() != NULL){
			$sql .= " id_usuario = ? AND";
	...
	public static final function byRange( $retencion_usuarioA , $retencion_usuarioB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from retencion_usuario WHERE ("; 
		$val = array();
		if( (($a = $retencion_usuarioA->getIdRetencion()) != NULL) & ( ($b = $retencion_usuarioB->getIdRetencion()) != NULL) ){
		...
		if( (($a = $retencion_usuarioA->getIdUsuario()) != NULL) & ( ($b = $retencion_usuarioB->getIdUsuario()) != NULL) ){
	...
	{
		if(self::getByPK($retencion_usuario->getIdRetencion(), $retencion_usuario->getIdUsuario()) === NULL) throw new Exception('Campo no encontrado.');
