...
abstract class ClienteDAOBase extends DAO
{
	...
	public static final function search( $cliente , $orderBy = null, $orden = 'ASC')
	{
		...
		if( $cliente->getLocalidad() != NULL){
			$sql .= " localidad = ? AND";
			array_push( $val, $cliente->getLocalidad() );
		}
	...
	private static final function create( &$cliente )
	{
		...
		if($ar == 0) return 0;
		/* save autoincremented value on obj */  $cliente->setIdCliente( $conn->Insert_ID() ); /*  */ 
		return $ar;
	}
	...
	public static final function byRange( $clienteA , $clienteB , $orderBy = null, $orden = 'ASC')
	{
		...
		if( (($a = $clienteA->getLocalidad()) != NULL) & ( ($b = $clienteB->getLocalidad()) != NULL) ){
