...
abstract class ClienteDAOBase extends DAO
{
	...
	public static final function search( $cliente , $orderBy = null, $orden = 'ASC')
	{
		...
		if( $cliente->getNumeroExteriror() != NULL){
			$sql .= " numero_exteriror = ? AND";
			array_push( $val, $cliente->getNumeroExteriror() );
		}
	...
	{
		$sql = "UPDATE cliente SET  rfc = ?, razon_social = ?, calle = ?, numero_exteriror = ?, numero_interior = ?, colonia = ?, referencia = ?, municipio = ?, estado = ?, pais = ?, codigo_postal = ?, telefono = ?, e_mail = ?, limite_credito = ?, descuento = ?, activo = ?, id_usuario = ?, id_sucursal = ?, fecha_ingreso = ? WHERE  id_cliente = ?;";
		$params = array( 
			$cliente->getRfc(), 
			$cliente->getRazonSocial(), 
			$cliente->getCalle(), 
			$cliente->getNumeroExteriror(), 
	...
	{
		$sql = "INSERT INTO cliente ( id_cliente, rfc, razon_social, calle, numero_exteriror, numero_interior, colonia, referencia, municipio, estado, pais, codigo_postal, telefono, e_mail, limite_credito, descuento, activo, id_usuario, id_sucursal, fecha_ingreso ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
		$params = array( 
			$cliente->getIdCliente(), 
			$cliente->getRfc(), 
			$cliente->getRazonSocial(), 
	...
	public static final function byRange( $clienteA , $clienteB , $orderBy = null, $orden = 'ASC')
	{
		...
		if( (($a = $clienteA->getNumeroExteriror()) != NULL) & ( ($b = $clienteB->getNumeroExteriror()) != NULL) ){
				$sql .= " numero_exteriror >= ? AND numero_exteriror <= ? AND";
				array_push( $val, min($a,$b)); 
				array_push( $val, max($a,$b)); 
		}elseif( $a || $b ){
			$sql .= " numero_exteriror = ? AND"; 
