...
abstract class DocumentoVentaDAOBase extends DAO
{
		...
		private static function recordExists(  $id_documento, $id_venta ){
			$pk = "";
			$pk .= $id_documento . "-";
			$pk .= $id_venta . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_documento, $id_venta){
			$pk = "";
			$pk .= $id_documento . "-";
			$pk .= $id_venta . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_documento, $id_venta ){
			$pk = "";
			$pk .= $id_documento . "-";
			$pk .= $id_venta . "-";
	...
	{
		if(  self::getByPK(  $documento_venta->getIdDocumento() , $documento_venta->getIdVenta() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_documento, $id_venta)){
			return self::getRecord( $id_documento, $id_venta );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_documento, $id_venta );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_documento
			//id_venta
    		self::pushRecord( $bar, $foo["id_documento"],$foo["id_venta"] );
		}
		return $allData;
	}
	...
	public static final function search( $documento_venta , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $documento_venta->getIdDocumento() != NULL){
			$sql .= " id_documento = ? AND";
			array_push( $val, $documento_venta->getIdDocumento() );
		}

		if( $documento_venta->getIdVenta() != NULL){
			$sql .= " id_venta = ? AND";
	...
	{
		if(self::getByPK($documento_venta->getIdDocumento(), $documento_venta->getIdVenta()) === NULL) throw new Exception('Campo no encontrado.');
