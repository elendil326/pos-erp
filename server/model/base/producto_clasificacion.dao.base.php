...
abstract class ProductoClasificacionDAOBase extends DAO
{
		...
		private static function recordExists(  $id_producto, $id_clasificacion_producto ){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_clasificacion_producto . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_producto, $id_clasificacion_producto){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_clasificacion_producto . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_producto, $id_clasificacion_producto ){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_clasificacion_producto . "-";
	...
	{
		if(  self::getByPK(  $producto_clasificacion->getIdProducto() , $producto_clasificacion->getIdClasificacionProducto() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_producto, $id_clasificacion_producto)){
			return self::getRecord( $id_producto, $id_clasificacion_producto );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_producto, $id_clasificacion_producto );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		self::pushRecord( $bar, $foo["id_producto"],$foo["id_clasificacion_producto"] );
		}
		return $allData;
	}
	...
	public static final function search( $producto_clasificacion , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $producto_clasificacion->getIdProducto() != NULL){
			$sql .= " id_producto = ? AND";
			array_push( $val, $producto_clasificacion->getIdProducto() );
		}

		if( $producto_clasificacion->getIdClasificacionProducto() != NULL){
	...
	public static final function byRange( $producto_clasificacionA , $producto_clasificacionB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from producto_clasificacion WHERE ("; 
		$val = array();
		if( (($a = $producto_clasificacionA->getIdProducto()) != NULL) & ( ($b = $producto_clasificacionB->getIdProducto()) != NULL) ){
		...
		if( (($a = $producto_clasificacionA->getIdClasificacionProducto()) != NULL) & ( ($b = $producto_clasificacionB->getIdClasificacionProducto()) != NULL) ){
	...
	{
		if(self::getByPK($producto_clasificacion->getIdProducto(), $producto_clasificacion->getIdClasificacionProducto()) === NULL) throw new Exception('Campo no encontrado.');
