...
abstract class ImpuestoDAOBase extends TablaDAO
{
	...
	/**
	  ...
	  **/
	public static final function search( $impuesto , $json = false)
	{
	...
	/**
	  ...
	  * @param bool [$json] Verdadero para obtener los resultados en forma JSON y no objetos. En caso de no presentare este parametro se tomara el valor default de false.
	  **/
	public static final function byRange( $impuestoA , $impuestoB , $json = false)
	{
		$sql = "SELECT * from impuesto WHERE ("; 
		$val = array();
		...
		if( (($a = $impuestoA->getValor()) != NULL) & ( ($b = $impuestoB->getValor()) != NULL) ){
		...
		if( (($a = $impuestoA->getIdSucursal()) != NULL) & ( ($b = $impuestoB->getIdSucursal()) != NULL) ){
