...
abstract class RetencionEmpresaDAOBase extends DAO
{
	...
	{
		if( ! is_null ( self::getByPK(  $retencion_empresa->getIdRetencion() , $retencion_empresa->getIdEmpresa() ) ) )
		{
	...
	{
		if(  is_null( $id_retencion ) || is_null( $id_empresa )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "RetencionEmpresa-" . $id_retencion."-" . $id_empresa ))){
                Logger::log("REDIS !");
                return new RetencionEmpresa($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "RetencionEmpresa-" . $id_retencion."-" . $id_empresa, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "RetencionEmpresa-" . $bar->getIdRetencion()."-" . $bar->getIdEmpresa(), $bar );
		}
		return $allData;
	}
	...
	public static final function search( $retencion_empresa , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( ! is_null( $retencion_empresa->getIdRetencion() ) ){
			$sql .= " `id_retencion` = ? AND";
			array_push( $val, $retencion_empresa->getIdRetencion() );
		}

		if( ! is_null( $retencion_empresa->getIdEmpresa() ) ){
			$sql .= " `id_empresa` = ? AND";
	...
	public static final function byRange( $retencion_empresaA , $retencion_empresaB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from retencion_empresa WHERE ("; 
		$val = array();
		if( ( !is_null (($a = $retencion_empresaA->getIdRetencion()) ) ) & ( ! is_null ( ($b = $retencion_empresaB->getIdRetencion()) ) ) ){
		...
		if( ( !is_null (($a = $retencion_empresaA->getIdEmpresa()) ) ) & ( ! is_null ( ($b = $retencion_empresaB->getIdEmpresa()) ) ) ){
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new RetencionEmpresa($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "RetencionEmpresa-" . $bar->getIdRetencion()."-" . $bar->getIdEmpresa(), $bar );
		}
		return $ar;
	}
	...
	{
		if( is_null( self::getByPK($retencion_empresa->getIdRetencion(), $retencion_empresa->getIdEmpresa()) ) ) throw new Exception('Campo no encontrado.');
