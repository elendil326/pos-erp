...
abstract class ChequeDAOBase extends DAO
{
		...
		private static function recordExists(  $id_cheque ){
			$pk = "";
			$pk .= $id_cheque . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_cheque){
			$pk = "";
			$pk .= $id_cheque . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_cheque ){
			$pk = "";
			$pk .= $id_cheque . "-";
	...
	{
		if(  self::getByPK(  $cheque->getIdCheque() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_cheque)){
			return self::getRecord( $id_cheque );
		}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_cheque
    		self::pushRecord( $bar, $foo["id_cheque"] );
		}
		return $allData;
	}
	...
	public static final function search( $cheque , $orderBy = null, $orden = 'ASC')
	{
		...
		if( $cheque->getExpedido() != NULL){
			$sql .= " expedido = ? AND";
			array_push( $val, $cheque->getExpedido() );
		}

		if( $cheque->getIdUsuario() != NULL){
			$sql .= " id_usuario = ? AND";
	...
	{
		if(self::getByPK($cheque->getIdCheque()) === NULL) throw new Exception('Campo no encontrado.');
