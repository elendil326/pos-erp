...
abstract class ChequeAbonoVentaDAOBase extends DAO
{
	...
	{
		if(  is_null( $id_cheque ) || is_null( $id_abono_venta )  ){ return NULL; }
            if(!is_null( self::$redisConection ) && !is_null($obj = self::$redisConection->get( "ChequeAbonoVenta-" . $id_cheque."-" . $id_abono_venta ))){
                Logger::log("REDIS !");
                return new ChequeAbonoVenta($obj);
            }
		...
		if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoVenta-" . $id_cheque."-" . $id_abono_venta, $foo );
		return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
    		...
    		array_push( $allData, $bar);
                if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoVenta-" . $bar->getIdCheque()."-" . $bar->getIdAbonoVenta(), $bar );
		}
		return $allData;
	}
	...
	public static final function byRange( $cheque_abono_ventaA , $cheque_abono_ventaB , $orderBy = null, $orden = 'ASC')
	{
		...
		$ar = array();
		foreach ($rs as $foo) {
    		array_push( $ar, $bar = new ChequeAbonoVenta($foo));
                    if(!is_null(self::$redisConection)) self::$redisConection->set(  "ChequeAbonoVenta-" . $bar->getIdCheque()."-" . $bar->getIdAbonoVenta(), $bar );
		}
		return $ar;
	}
