...
abstract class PrecioProductoUsuarioDAOBase extends DAO
{
	...
	{
		if( ! is_null ( self::getByPK(  $precio_producto_usuario->getIdProducto() , $precio_producto_usuario->getIdUsuario() ) ) )
		{
	...
	public static final function search( $precio_producto_usuario , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( ! is_null( $precio_producto_usuario->getIdProducto() ) ){
			$sql .= " id_producto = ? AND";
			array_push( $val, $precio_producto_usuario->getIdProducto() );
		}

		if( ! is_null( $precio_producto_usuario->getIdUsuario() ) ){
			$sql .= " id_usuario = ? AND";
			array_push( $val, $precio_producto_usuario->getIdUsuario() );
		}

		if( ! is_null( $precio_producto_usuario->getEsMargenUtilidad() ) ){
			$sql .= " es_margen_utilidad = ? AND";
			array_push( $val, $precio_producto_usuario->getEsMargenUtilidad() );
		}

		if( ! is_null( $precio_producto_usuario->getPrecioUtilidad() ) ){
	...
	public static final function byRange( $precio_producto_usuarioA , $precio_producto_usuarioB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from precio_producto_usuario WHERE ("; 
		$val = array();
		if( ( !is_null (($a = $precio_producto_usuarioA->getIdProducto()) ) ) & ( ! is_null ( ($b = $precio_producto_usuarioB->getIdProducto()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_usuarioA->getIdUsuario()) ) ) & ( ! is_null ( ($b = $precio_producto_usuarioB->getIdUsuario()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_usuarioA->getEsMargenUtilidad()) ) ) & ( ! is_null ( ($b = $precio_producto_usuarioB->getEsMargenUtilidad()) ) ) ){
		...
		if( ( !is_null (($a = $precio_producto_usuarioA->getPrecioUtilidad()) ) ) & ( ! is_null ( ($b = $precio_producto_usuarioB->getPrecioUtilidad()) ) ) ){
	...
	{
		if( is_null( self::getByPK($precio_producto_usuario->getIdProducto(), $precio_producto_usuario->getIdUsuario()) ) ) throw new Exception('Campo no encontrado.');
