...
abstract class PagosVentaDAOBase extends TablaDAO
{
	...
	/**
	  ...
	  **/
	public static final function search( $pagos_venta , $json = false)
	{
	...
	{
		$sql = "INSERT INTO pagos_venta ( id_venta, id_sucursal, id_usuario, fecha, monto ) VALUES ( ?, ?, ?, ?, ?);";
		$params = array( 
			$pagos_venta->getIdVenta(), 
			$pagos_venta->getIdSucursal(), 
			$pagos_venta->getIdUsuario(), 
	...
	/**
	  ...
	  * @param bool [$json] Verdadero para obtener los resultados en forma JSON y no objetos. En caso de no presentare este parametro se tomara el valor default de false.
	  **/
	public static final function byRange( $pagos_ventaA , $pagos_ventaB , $json = false)
	{
		$sql = "SELECT * from pagos_venta WHERE ("; 
		$val = array();
