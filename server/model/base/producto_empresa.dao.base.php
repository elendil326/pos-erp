...
abstract class ProductoEmpresaDAOBase extends DAO
{
		...
		private static function recordExists(  $id_producto, $id_empresa ){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_empresa . "-";
			return array_key_exists ( $pk , self::$loadedRecords );
		}
		private static function pushRecord( $inventario,  $id_producto, $id_empresa){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_empresa . "-";
			self::$loadedRecords [$pk] = $inventario;
		}
		private static function getRecord(  $id_producto, $id_empresa ){
			$pk = "";
			$pk .= $id_producto . "-";
			$pk .= $id_empresa . "-";
	...
	{
		if(  self::getByPK(  $producto_empresa->getIdProducto() , $producto_empresa->getIdEmpresa() ) !== NULL )
		{
	...
	{
		if(self::recordExists(  $id_producto, $id_empresa)){
			return self::getRecord( $id_producto, $id_empresa );
		}
		...
		if(count($rs)==0)return NULL;
			...
			self::pushRecord( $foo,  $id_producto, $id_empresa );
			return $foo;
	}
	...
	public static final function getAll( $pagina = NULL, $columnas_por_pagina = NULL, $orden = NULL, $tipo_de_orden = 'ASC' )
	{
		...
		foreach ($rs as $foo) {
			...
			//id_producto
			//id_empresa
    		self::pushRecord( $bar, $foo["id_producto"],$foo["id_empresa"] );
		}
		return $allData;
	}
	...
	public static final function search( $producto_empresa , $orderBy = null, $orden = 'ASC')
	{
		...
		$val = array();
		if( $producto_empresa->getIdProducto() != NULL){
			$sql .= " id_producto = ? AND";
			array_push( $val, $producto_empresa->getIdProducto() );
		}

		if( $producto_empresa->getIdEmpresa() != NULL){
			$sql .= " id_empresa = ? AND";
			array_push( $val, $producto_empresa->getIdEmpresa() );
		}

		if( $producto_empresa->getPrecioUtilidad() != NULL){
			$sql .= " precio_utilidad = ? AND";
			array_push( $val, $producto_empresa->getPrecioUtilidad() );
		}

		if( $producto_empresa->getEsMargenUtilidad() != NULL){
			$sql .= " es_margen_utilidad = ? AND";
			array_push( $val, $producto_empresa->getEsMargenUtilidad() );
		}
	...
	{
		$sql = "UPDATE producto_empresa SET  precio_utilidad = ?, es_margen_utilidad = ? WHERE  id_producto = ? AND id_empresa = ?;";
		$params = array( 
			$producto_empresa->getPrecioUtilidad(), 
			$producto_empresa->getEsMargenUtilidad(), 
			$producto_empresa->getIdProducto(),$producto_empresa->getIdEmpresa(), );
		global $conn;
	...
	{
		$sql = "INSERT INTO producto_empresa ( id_producto, id_empresa, precio_utilidad, es_margen_utilidad ) VALUES ( ?, ?, ?, ?);";
		$params = array( 
	...
	public static final function byRange( $producto_empresaA , $producto_empresaB , $orderBy = null, $orden = 'ASC')
	{
		$sql = "SELECT * from producto_empresa WHERE ("; 
		$val = array();
		if( (($a = $producto_empresaA->getIdProducto()) !== NULL) & ( ($b = $producto_empresaB->getIdProducto()) !== NULL) ){
		...
		if( (($a = $producto_empresaA->getIdEmpresa()) !== NULL) & ( ($b = $producto_empresaB->getIdEmpresa()) !== NULL) ){
		...
		if( (($a = $producto_empresaA->getPrecioUtilidad()) !== NULL) & ( ($b = $producto_empresaB->getPrecioUtilidad()) !== NULL) ){
		...
		if( (($a = $producto_empresaA->getEsMargenUtilidad()) !== NULL) & ( ($b = $producto_empresaB->getEsMargenUtilidad()) !== NULL) ){
	...
	{
		if(self::getByPK($producto_empresa->getIdProducto(), $producto_empresa->getIdEmpresa()) === NULL) throw new Exception('Campo no encontrado.');
