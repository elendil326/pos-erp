...
class ViewGastosDAO extends ViewGastosDAOBase
{
	...
	*/
	...
	static function gastosSucursal( $timeRange, $fechaInicio, $fechaFinal, $id_sucursal )
	{
	
		$allVentas = ViewGastosDAO::getAllGastos( $timeRange, $fechaInicio, $fechaFinal, $id_sucursal );
		...
		for ( $i = 0; $i < count($allVentas) ; $i++ )
		{
			...
			{
				$viewGastos = new ViewGastos();
				$viewGastos->setIdSucursal($id);
				if ( $timeRange != null || $fechaInicio != null || $fechaFinal != null )
				{
					$ventasFecha1->setIdSucursal($id);
					$ventasFecha2->setIdSucursal($id);
					$sucursalVentas = ViewGastosDAO::byRange( $ventasFecha1, $ventasFecha2 );
				}
				else
				{
					$sucursalVentas = ViewGastosDAO::search( $viewGastos );
				}
				//echo "no duplicado";
