	...
	*/
	...
	function selectEfectivoIntervalo( $interval, $table )
	{
		...
		//La clausula SELECT, FROM y WHERE la agregamos nosotros, esta es la que cambia
			switch( $interval ) 
			{
				case 'semana'	: $qry = "SELECT DAYOFWEEK(`".$table."`.`fecha`) AS `x`, SUM(`monto`) AS `y`, DAYNAME(`".$table."`.`fecha`) AS `label` FROM `".$table."`  ";
						break;
				case 'mes'	: $qry = "SELECT DAYOFMONTH(`".$table."`.`fecha`) AS `x`, SUM(`monto`) AS `y`, DAYOFMONTH(`".$table."`.`fecha`) AS `label` FROM `".$table."` ";
						break;
				case 'year'	: $qry = "SELECT MONTH(`".$table."`.`fecha`) AS `x`, SUM(`monto`) AS `y`, MONTHNAME(`".$table."`.`fecha`) AS `label` FROM `".$table."` ";
						break;
				default: break;
			}
	...
	*/
	function graficaComprasCredito(){
		...
		if( isset( $_REQUEST['dateRange']) )
		{
			...
			//Formamos nuestro query completo
			$completeQuery = $functionQuery . ' `ventas`.`tipo_compra` = 2 AND ' . $qry_select ;
	...
	*/
	...
	function graficaGastos()
	{
		...
		if( isset( $_REQUEST['dateRange']) )
		{
			...
			if ( isset( $_REQUEST['id_sucursal'] ) )
			{
				$functionQuery .= " WHERE `gastos`.`id_sucursal` = ? AND ";
				array_push( $params, $_REQUEST['id_sucursal'] );
				array_push( $params, $datesArray[0] );
				array_push( $params, $datesArray[1] );
			}
	...
	*/
	...
	function graficaIngresos()
	{
		...
		if( isset( $_REQUEST['dateRange']) )
		{
			...
			if ( isset( $_REQUEST['id_sucursal'] ) )
			{
				$functionQuery .= " WHERE `ingresos`.`id_sucursal` = ? AND ";
				array_push( $params, $_REQUEST['id_sucursal'] );
				array_push( $params, $datesArray[0] );
				array_push( $params, $datesArray[1] );
			}
