...
class ImpresionesController {
	...
	private static function moneyFormat($foo){
		return $foo;
	}
	...
	public static function imprimirFactura($id_venta, $venta_especial = null) {
	     ...
	     * Datos del receptor
	     * ************************* */
	    $datos_receptor = self::readableText($cliente->getRazonSocial()) . "\n";
	    ...
	    $comprobante = array(
	        array("r1" => "Tipo de comprobante",
	            ...
	            "r3" => "Tipo de cambio",
	            "r4" => "<b>Version</b>"),
	        array("r1" => self::readableText($factura->getTipoComprobante()),
	            "r2" => "MXN",
	            "r3" => "1.0",
	            "r4" => "3.0"),
	    );
	...
	public static function imprimirFacturaXML($id_venta, $venta_especial = null) {
	    ...
	    $e = "<b>" . self::readableText($xml->Emisor['nombre']) . "</b>\n";
	    ...
	    $datos_receptor = self::readableText($xml->Receptor['nombre']) . "\n";
	    ...
	    $pagare = "\n\nNo. _________                                                                                                                                                                                    En " . self::readableText($xml->Emisor->DomicilioFiscal['municipio']) . " a " . date("d", strtotime( $fecha_emision )) . " de " . $mes . " del " . date("Y", strtotime( $fecha_emision )) ."\n\n";
	    $pagare .= " Debe(mos) y pagare(mos) incondicionalmente por este Pagaré a la orden de " . self::readableText($xml->Emisor['nombre']) . " en " . self::readableText($xml->Emisor->DomicilioFiscal['municipio']) . " " . self::readableText($xml->Emisor->DomicilioFiscal['estado']) . " ";
	...
	private static function obternerQRCode($rfcEmisor = null, $rfcReceptor = null, $total = null, $uuid = null) {
	...
	public static function leerLeyendasTicket() {
