...
try{window.webkitNotifications.requestPermission();}catch(e){console.error(e)}
function notification(){if(window.webkitNotifications.checkPermission()==0){window.webkitNotifications.createNotification("","Autorizaciones","Usted tiene nuevas autorizaciones por atender").show();}else{window.webkitNotifications.requestPermission();}}
var auths_hash=null;function heartbeat(){jQuery.ajaxSettings.traditional=true;jQuery.ajax({url:"../proxy.php",data:{action:207,hashCheck:auths_hash},cache:false,success:function(data){try{response=jQuery.parseJSON(data);if(response==null){return;}
console.log("Heart beat is not empty !",response)
if(response.reboot){console.warn("reboot!");window.location="../proxy.php?action=2002";return;}
if((auths_hash!=null)&&(auths_hash!=response.hash)){notification();}
auths_hash=response.hash;}catch(e){}}});};setInterval("heartbeat()",15000);function meses(m){m=parseFloat(m);switch(m){case 1:return"enero";case 2:return"febrero";case 3:return"marzo";case 4:return"abril";case 5:return"mayo";case 6:return"junio";case 7:return"julio";case 8:return"agosto";case 9:return"septiembre";case 10:return"octubre";case 11:return"noviembre";case 12:return"diciembre";}}
POS={};POS.ajaxToClient=function(module,args,callback)
{Ext.Ajax.request({url:'http://localhost:16001/',params:{action:module,data:args},scriptTag:true,success:function(r){callback.call(null,r.responseObject);}});}
if(window.Ext)
...
this.fireEvent("requestcomplete",this,response,options);Ext.callback(options.success,options.scope,[response,options]);Ext.callback(options.callback,options.scope,[options,true,response]);},handleScriptFailure:function(trans){this.transId=false;this.destroyScriptTrans(trans,false);var options=trans.options;response={argument:options.argument,status:500,statusText:'Server failed to respond',responseText:''};this.fireEvent("requestexception",this,response,options,{status:-1,statusText:'communication failure'});Ext.callback(options.failure,options.scope,[response,options]);Ext.callback(options.callback,options.scope,[options,false,response]);},destroyScriptTrans:function(trans,isLoaded){document.getElementsByTagName("head")[0].removeChild(document.getElementById(trans.scriptId));clearTimeout(trans.timeoutId);if(isLoaded){window[trans.cb]=undefined;try{delete window[trans.cb];}catch(e){}}else{window[trans.cb]=function(){window[trans.cb]=undefined;try{delete window[trans.cb];}catch(e){}};}}});});}
function tr(s,o){if(o){return"<tr "+o+">"+s+"</tr>";}else{return"<tr >"+s+"</tr>";}}
function td(s,o){if(o){return"<td "+o+">"+s+"</td>";}else{return"<td >"+s+"</td>";}}
function div(s,o){if(o){return"<div "+o+">"+s+"</div>";}else{return"<div >"+s+"</div>";}}
...
{return(num<<cnt)|(num>>>(32-cnt));}cf=function(num){if(num==null)
num=0;num=num.toString().replace(/\$|\,/g,'');if(isNaN(num)){num="0";}
...
return(((sign)?'':'-')+'$'+num+'.'+cents);};var Url={encode:function(string){return escape(this._utf8_encode(string));},decode:function(string){return this._utf8_decode(unescape(string));},_utf8_encode:function(string){string=string.replace(/\r\n/g,"\n");var utftext="";for(var n=0;n<string.length;n++){var c=string.charCodeAt(n);if(c<128){utftext+=String.fromCharCode(c);}
